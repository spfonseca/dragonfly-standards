...
[REQUIRED] Request and response (custom). Every request shall carry exactly one Request-Id custom header following the `<custom name>-<header name>` pattern (§10.2), assigned at the outermost edge the request passes through — the first component able to reject it — or by the receiving API when a request originating inside the platform arrives without one, then propagated unchanged across the full request/response path (§22.1) and echoed on the response. Where the edge already assigns a distributed-trace identifier, the Request-Id shall be derived from it rather than independently generated. A client-supplied Request-Id shall be recorded for the client's own correlation but never adopted as the request's identifier, and any identifier an intermediary mints for its internal diagnostics is not the Request-Id and shall not appear in a contract. *Rationale:* Assigning at the outermost edge is what makes a request rejected before it reaches any API — by the firewall, the gateway, or a rate limiter — traceable on the same identifier as one that succeeds; deriving it from the trace identifier keeps the value a consumer quotes joinable to logs and traces without translation; and declining a client-supplied value preserves the uniqueness a caller repeating or forging one would destroy. *Example:* `Dragonfly-Request-Id: 4bf92f3577b34da6a3ce929d0e0e4736`
...
[REQUIRED] Each request/response shall be associated with its unique Request-Id (§10.19), surfaced to consumers as the error supportId, carried through internal processing, and recorded in structured logs together with the distributed-trace identifier it derives from, so that a Request-Id quoted by a consumer resolves to that request's logs and its trace without an intermediate lookup. *Rationale:* A per-request identifier carried end to end is what makes a single failing request traceable across services; recording it alongside the trace identifier is what keeps a support conversation, an access log, and a trace joinable as one investigation instead of three.
