...
### 7.13 Name a scoped attribute for its concept, then its scope, then its state.

[REQUIRED] Where the same question is answered at more than one scope — the platform’s answer, a tenant’s, and the one actually in force — each attribute shall be named `<concept><scope><state>`, with the scope in the middle and the state as the suffix, in camelCase (§11.1). The scope segment is `Platform`, `Organization` or `Effective`. The effective attribute is read-only, server-computed, and is what a consumer reads: the resolution rule belongs to the API, not to every client. *Rationale:* A consistent order makes the three readable at a glance and keeps unrelated states — required, enabled, permitted — from being confused with one another, which a suffixed scope (`mfaRequiredPlatform`) obscures by putting the varying part last. *Example:* `mfaPlatformRequired`, `mfaOrganizationRequired`, `mfaEffectiveRequired`; `notificationsPlatformEnabled`, `notificationsOrganizationEnabled`, `notificationsEffectiveEnabled`
...
[REQUIRED] Request and response. Every payload shall declare its representation via a correct Content-Type carrying a standard, IANA-registered media type: application/json for representations, application/problem+json for error bodies (§12.17), JSON or HTML for the metadata endpoints. *Rationale:* Self-describing payloads let any client and intermediary interpret content without out-of-band knowledge.
...
[REQUIRED] Request and response (custom). Every request shall carry exactly one Request-Id custom header following the `<custom name>-<header name>` pattern (§10.2), generated at the first platform-controlled component the request cannot bypass — the gateway for externally originated traffic, the receiving API for calls originating inside the platform — then propagated unchanged across the full request/response path (§22.1) and echoed on the response. The value shall be generated by that component, never derived from a header a caller can set, and shall be recorded alongside whatever distributed-trace identifier accompanied the request (§22.1). A client-supplied Request-Id shall never be adopted as the request's identifier, though it may be recorded for the caller's own correlation, and any identifier an intermediary mints for its internal diagnostics is not the Request-Id and shall not appear in a contract. *Rationale:* Uniqueness is what makes this value usable as the error supportId (§12.19), and only an identifier the platform generates is unique — an edge trace identifier reads as authoritative but is client-settable, so a caller could collapse every request onto one support handle or replay another's. Recording it beside the trace identifier preserves the join to traces and to edge access logs without making the quoted string depend on caller input. *Example:* `Dragonfly-Request-Id: 4bf92f3577b34da6a3ce929d0e0e4736`
...
[REQUIRED] Every implementation shall catch every exception and return the standard error body, based on RFC 9457 Problem Details — type, title, status, and detail as the RFC defines them — plus the supportId extension member (§12.19). type is a URI identifying the specific error condition, dereferencing to its documentation, and treated by clients as an opaque, permanent identifier. supportId is always equal to the Request-Id of the failing request (§10.19). Every error response shall declare Content-Type: application/problem+json (§10.6), while success responses remain application/json, so the media type alone distinguishes an error payload from a representation. This body is required of every error a consumer can receive from the API or from the API gateway answering on its behalf, including gateway-originated errors raised before the request reaches the implementation. *Rationale:* One RFC-based error shape lets every consumer parse failures with a single code path; status carries the standardized coarse classification, the type URI is at once the branchable identifier and the documentation address, and the distinct media type lets any client or intermediary recognize the standard error shape without prior knowledge of the API. Binding the gateway to the same shape matters most of all, since a gateway error is the one failure a service cannot fix and the consumer cannot influence.
...
[REQUIRED] Each request/response shall be associated with its unique Request-Id (§10.19), surfaced to consumers as the error supportId, carried through internal processing, and recorded in structured logs together with whatever distributed-trace identifier accompanied the request, so that a Request-Id quoted by a consumer resolves to that request's logs and, through them, to its trace. *Rationale:* A per-request identifier carried end to end is what makes a single failing request traceable across services; recording it alongside the trace identifier is what keeps a support conversation, an access log, and a trace joinable as one investigation instead of three, without the quoted identifier having to be a value the caller could have set.
