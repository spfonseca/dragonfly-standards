...
| [RESTful API Design Standard v1.1 (draft)](rest-api-design/RESTful-API-Design-Standard-v1.1.md) | Working draft of the above — holds guidelines still under elaboration (the shared batch idiom §14.7). Do not build against: a guideline moves down into v1.0 when it is agreed *and* being built, so v1.1 holds only what is agreed for later. | Draft |
